...
EOF
  ...
  # also install classic GRUB stage 1 and 2 bootloaders in case we are booting from PV-GRUB on AWS
  mkdir -p ${image_mount_point}/boot/grub
  cp ${dir}/assets/x86_64-redhat/stage? ${image_mount_point}/boot/grub/
...
title ${os_name} (${kernel_version})
  root (hd0,0)
  kernel /boot/vmlinuz-${kernel_version} ro root=UUID=${uuid} selinux=0 cgroup_enable=memory swapaccount=1 console=tty0 console=ttyS0,115200n8 net.ifnames=0 plymouth.enable=0
